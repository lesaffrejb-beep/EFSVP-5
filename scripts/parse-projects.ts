...
function toEmbedUrl(url: string): string | null {
  ...
  const youtubeWatchMatch = trimmed.match(/youtube\.com\/watch\?v=([a-zA-Z0-9_-]+)/);
...
function parseRawProject(block: string): RawProject {
  ...
  const video = extractField(mediaSection, 'Vidéo (YouTube / Vimeo)');
...
function transformProject(raw: RawProject): Project {
  ...
  const videoEmbedUrl = raw.video ? toEmbedUrl(raw.video) : null;
  const hasVideo = Boolean(videoEmbedUrl || raw.video);
