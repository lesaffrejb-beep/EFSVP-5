...
--section-bg-default: var(--neutral-50)
--section-bg-soft: var(--neutral-100)
--section-bg-strong: #ffffff
```

**Arrière-plans de section** :
- Par défaut les sections reposent sur `--section-bg-default`.
- Les blocs impact / process / offres utilisent `--section-bg-soft` pour créer un chapitrage visuel.
- Les cartes spéciales (A2MO) peuvent rester sur `--section-bg-strong` pour laisser respirer le gradient.
- Section impact ("Ce que ça change vraiment") : `--section-bg-impact` pour un fond doux légèrement détaché.
- Section A2MO : `--section-bg-a2mo` (fond sombre home cinéma) + textes `--text-on-dark`.
...
| Token | Clamp min | Clamp max | Usage |
|-------|-----------|-----------|-------|
| `--text-xs` | 0.75rem | 0.85rem | Annotations, microcopy |
| `--text-sm` | 0.85rem | 0.95rem | Labels, tags |
| `--text-base` | 1rem | 1.1rem | Corps de texte |
| `--text-lg` | 1.1rem | 1.4rem | Paragraphes d'accroche |
| `--text-xl` | 1.4rem | 1.9rem | Titres de cartes |
| `--text-2xl` | 1.8rem | 2.5rem | Titres de section (H2) |
| `--text-3xl` | 2.2rem | 3.2rem | Hero (H1 visible) |
| `--text-4xl` | 2.25rem | 2.25rem | Valeurs héritées (display) |
| `--text-5xl` | 3rem | 3rem | Valeurs héritées (display) |
| `--text-6xl` | 3.75rem | 3.75rem | Valeurs héritées (display) |
...
**Mapping rapide** :
- H1 hero visible → `var(--text-3xl)` avec letter-spacing léger négatif et line-height 1.1.
- H2 de section → `var(--text-2xl)` line-height ~1.2.
- Titres de cartes (projets, process, impacts, tarifs) → `var(--text-xl)`.
- Corps de texte → `var(--text-base)` ; méta et tags → `var(--text-sm)` / `var(--text-xs)`.
...
- Padding vertical standard de section : `var(--section-spacing)` (mobile ≈ `--space-16`), `var(--section-spacing-lg)` (desktop ≈ `--space-24`).
- Éviter les écarts démesurés entre FAQ et Contact : conserver le rythme 16/24 sauf besoin exceptionnel.
...
- La section "Quelques histoires que nous avons racontées" est l'unique point d'entrée du portfolio sur la home.
- Règle d'affichage : 6 projets en premier rendu, puis le bouton secondaire/ghost `#see-all-projects` (label : "Voir tous nos projets") dévoile l'intégralité des cartes dans la même grille.
- Une fois tous les projets affichés, le CTA est désactivé/masqué pour éviter les actions inutiles.
...
**Micro-interactions** :
- Boutons primaires : translation -2px au hover, gloss `::before` animé.
- Cartes projets/process/tarifs : lift + ombre `--shadow-xl`, zoom léger sur visuels.
- Respect du `prefers-reduced-motion` : transitions forcées à 0.01ms et scroll-behavior auto.
