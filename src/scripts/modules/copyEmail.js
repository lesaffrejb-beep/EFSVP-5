...
export class CopyEmail {
  ...
  showFeedback(buttonData, state) {
    ...
    const feedback = button.closest('.contact__direct')?.querySelector('[data-copy-feedback]');
    ...
    if (feedback) {
      const shouldShow = state === 'success';
      feedback.hidden = !shouldShow;
      feedback.classList.toggle('is-visible', shouldShow);
