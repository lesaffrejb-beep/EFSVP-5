...
function renderAudioPlayerHTML(container, project) {
  ...
  const contentEl = renderAudioShell(container);
...
function renderAudioShell(container) {
  container.innerHTML = `
    <section class="project-audio" aria-label="Lecteur audio du projet">
      <div class="project-audio__header">
        <span class="tag project-audio__eyebrow">Extrait audio</span>
      </div>
      <div class="project-audio__card">
        <div class="project-audio__content" data-audio-content></div>
      </div>
    </section>
  `;

  const contentEl = container.querySelector('[data-audio-content]');
  if (!contentEl) {
    throw new Error('[ProjectAudioPlayer] Impossible de créer le conteneur audio');
  }
...
function initWaveSurfer(container, project) {
  ...
  if (typeof audioPath !== 'string' || !audioPath.trim()) {
    throw new Error(`[ProjectAudioPlayer] audioPath invalide pour ${project.id}`);
  }
  ...
  wavesurfer.on('decodeerror', (error) => {
    console.error('[ProjectAudioPlayer] Decode error:', error);
    hideLoadingState(container);
    showAudioError(container.closest('.audio-player'));
  });
