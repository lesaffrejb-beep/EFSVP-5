...
export function initPortfolioContent() {
  ...
  const featuredTitle = document.querySelector('[data-projects-title]');
  if (featuredTitle && projects?.title) {
    featuredTitle.textContent = projects.title;
  }

  const allTitle = document.querySelector('[data-all-projects-title]');
  if (allTitle && projects?.allTitle) {
    allTitle.textContent = projects.allTitle;
  }

  const viewAllButtons = document.querySelectorAll('[data-projects-view-all]');
  viewAllButtons.forEach((button) => {
    if (projects?.viewAllLabel) {
      button.textContent = projects.viewAllLabel;
    }
  });
}
