...
import { getProjectMedia } from '@/data/projectMedia.loader';
...
export function createProjectCard({ project, onSelect }: ProjectCardProps): HTMLElement {
  ...
  let previewImage: string | null = project.thumbnailSrc || null;

  try {
    const media = getProjectMedia(project.slug);
    if (media.heroImage) {
      previewImage = media.heroImage;
    }
  } catch (error) {
    console.error('[ProjectCard] Impossible de charger les médias du projet', project.slug, error);
  }
